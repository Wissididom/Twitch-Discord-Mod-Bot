	...
	return new Promise(async (resolve, reject) => {
		try {
			...
			if (json.error) {
				...
				const channelPointsVoting = data.channel_points_voting_enabled ? strings['enabled'] : strings['disabled'];
				...
				response.push(strings.poll['channel-points-voting'].replace('<channelPointsVoting>', channelPointsVoting));
				response.push(`${strings.poll['status']}: ${data.status}`);
				response.push(`${strings.poll['duration']}: ${data.duration} seconds`);
				response.push(strings.poll['started-at'].replace('<startedAt>', toDiscordTimestamp(data.started_at)));
			}
...
async function createPoll(clientId, accessToken, broadcasterId, title, choices, duration, channelPointsVotingEnabled, channelPointsPerVote, strings) {
	return new Promise(async (resolve, reject) => {
		try {
...
async function endPoll(clientId, accessToken, broadcasterId, pollId, status, strings) {
	return new Promise(async (resolve, reject) => {
		try {
			const res = await fetch('https://api.twitch.tv/helix/polls', {
				method: 'PATCH',
			...
			if (json.error) {
				...
				response.push(`${strings.poll['duration']}: ${data.duration} seconds`);
				response.push(strings.poll['started-at'].replace('<startedAt>', toDiscordTimestamp(data.started_at)));
				response.push(strings.poll['ended-at'].replace('<endedAt>', toDiscordTimestamp(data.ended_at)));
			}
			resolve(response.join("\n"));
		} catch (e) {
			reject(e);
