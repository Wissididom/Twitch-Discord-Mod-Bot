import { toDiscordTimestamp } from "./util.js";
    ...
    try {
      ...
      if (json.error) {
        ...
        for (let i = 0; i < data.outcomes.length; i++) {
          ...
          outcomes.push(
            `> > ${strings.prediction.outcome["id"]}: ${outcome.id}`,
          );
          outcomes.push(
  ...
  return new Promise(async (resolve, reject) => {
    try {
      ...
      if (json.error) {
        ...
        let data = json.data[0];
        response.push(
          strings.prediction["started"].replace("<title>", data.title),
        );
        let outcomes = [];
        for (let i = 0; i < data.outcomes.length; i++) {
          let outcome = data.outcomes[i];
  ...
  return new Promise(async (resolve, reject) => {
    try {
      ...
      if (json.error) {
        ...
        let data = json.data[0];
        response.push(
          strings.prediction["ended"].replace("<title>", data.title),
        );
        let outcomes = [];
        for (let i = 0; i < data.outcomes.length; i++) {
          let outcome = data.outcomes[i];
        ...
        response.push(
          `${strings.prediction["outcomes"]}Outcomes:\n${outcomes}`,
        );
        response.push(
          `${strings.prediction["prediction-window"]}: ${data.prediction_window} seconds\n`,
        );
